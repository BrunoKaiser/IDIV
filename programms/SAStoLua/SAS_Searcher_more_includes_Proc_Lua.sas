...
searchText=" person "
fileTable={}
fileTable[#fileTable+1]="\\\\Client\\C$\\Temp\\SAS_Analyser.sas"
fileTable[#fileTable+1]="\\\\Client\\C$\\Temp\\SAS_Analyser_2.sas"
fileTable[#fileTable+1]="\\\\Client\\C$\\Temp\\test\\SAS_Analyser_2.sas"
	...
	for semikolon in (text .. "\n")
		...
		elseif semikolon:lower():match("include[ ]*=[ ]*.*\\[^,]*") then
			...
			for field in semikolon:gsub(";"," ;"):gsub(" +"," "):gmatch('[^=;]*') do
				if field:match("$\\") then
					print(numberProgramm .. ". Pfad include=: " .. field)
					DateiText=field:match('\\\\[cC][lL][iI][eE][nN][tT]\\.$\\[^=;]*%.sas')
		...
		elseif (" " .. semikolon):lower():match(searchText:lower()) then
			if uniqueTable[SASFile]==nil then
				if inputfile then
					...
					resultTable[#resultTable+1]=SASFile
				end --if inputfile then
...
outputFile=io.open("\\\\Client\\C$\\Temp\\SAS_searcher.txt","w")
...
os.execute('start "d" "\\\\Client\\C$\\Temp\\SAS_searcher.txt"')
...
for i,v in ipairs(resultTable) do
	print(v)
end --for i,v in ipairs(resultTable) do
